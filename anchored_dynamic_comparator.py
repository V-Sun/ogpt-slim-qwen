#!/usr/bin/env python3
"""Anchored pairwise comparator for cached SWE-bench patches.

By default this is pre-eval blind: the model sees issue text, FAIL_TO_PASS test
names, candidate diffs, and optionally a separately provided baseline trace. It
does not see candidate post-patch harness outcomes before making a decision.
"""
...
DEFAULT_OUTPUT_DIR = REPO / "outputs" / "anchored_dynamic_comparator_pilot"
...
patch text, the issue description, and the failure behavior of the unpatched code
only.
...
Most plausible patches are wrong.

Issue:
{issue_text}

FAIL_TO_PASS tests (these tests must pass after a correct patch):
{fail_to_pass_tests}

Failure behavior of the unpatched code (this is the baseline; both candidates were
built to address this failure):
...
Decision rules:
- Reason about whether each patch addresses the failure mode shown in the baseline trace.
- Prefer the patch that more directly fixes the cause of the listed FAIL_TO_PASS test failures.
- Reject patches that look unrelated to the issue, modify the wrong file, or introduce obvious regressions.
- Use TIE only if both candidates look genuinely equivalent in their likelihood of passing.
...
def build_prompt(
    ...
    # PRE-EVAL BLIND comparator (2026-05-05): the prompt now omits all per-candidate
    # post-patch behavior fields. The comparator picks which patch to send to the
    # harness; no eval has occurred yet from its point of view. Allowed signals:
    # issue text, FAIL_TO_PASS test names, baseline failure trace from the unpatched
    # repo, and the candidate diffs.
    return PROMPT_TEMPLATE.format(
        ...
        patch_a_diff=compact(pa, 8000),
        patch_b_changed_files="\n".join(f"- {x}" for x in patch_files(pb)) or "(none parsed)",
        patch_b_diff=compact(pb, 8000),
    )
